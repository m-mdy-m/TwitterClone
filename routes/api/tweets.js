...
router.group("/api", (r) => {
  ...
  r.setRoute("/create").using([ensureAuthenticated,verifyToken]).post(postTweet);

  // Defining a GET route for fetching tweets
  r.setRoute("/tweets").using([ensureAuthenticated,verifyToken]).get(getTweets);

  // Defining a PUT route for liking a tweet
  r.setRoute("/like/:id").using([ensureAuthenticated,verifyToken]).put(putLike);
});
