...
export function TweetsProfile(){
    return `
    <div class="max-w-full min-h-44 min-w-52 h-auto my-4 rounded-lg relative flex flex-col border-2 bg-[#11101076] border-t border-[#111010] p-2">
        <!-- Bookmarked Tweet  -->
        <svg class="bookmarked w-4 h-4 top-2 left-1 block absolute text-blue-400 transition-colors duration-300 ease-in-out" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="flex absolute top-0 left-1/3  justify-center items-center text-gray-500">
        ...
        <div class="flex justify-end items-center min-w-20 list__menu-icon h-auto [&>*]:cursor-pointer relative">
            <!-- Delete tweet icon -->
            <svg class="deleteIcon  w-6 h-6 text-gray-400 hover:text-red-500 transition-colors duration-300 ease-in-out" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <div class="min-h-44 max-h-44 max-w-52 flex justify-between items-start flex-col pb-3 pr-2 relative">
        ${PostContent({ profile_mode:true, content:"hi men Lorem ipsum dolor sit amet consectetur adipisicing elit. Obcaecati error, molestiae nihil ut autem praesentium sapiente quo a rem! Expedita, corrupti. Illo in velit animi repellat veniam qui voluptas mollitia!" })}
